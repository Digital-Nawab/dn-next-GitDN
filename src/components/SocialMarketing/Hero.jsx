    ...
    <section className="min-h-screen bg-gradient-to-br py-24 from-rose-100 via-pink-50 to-orange-100 relative overflow-hidden flex items-center px-4 md:px-8">
      ...
      <div className="absolute inset-0 hidden sm:block">
        ...
        {socialIcons.map((icon, index) => (
          ...
          >
            <div
              className={`w-10 h-10 md:w-12 md:h-12 ${icon.color} rounded-xl shadow-lg flex items-center justify-center hover:scale-110 transition-transform duration-300`}
            >
              {icon.name === "Facebook" && (
        ...
        {/* Decorative Stars */}
        <div
          className="absolute top-16 left-16 animate-spin"
          style={{ animationDuration: "8s" }}
        >
          <Sparkles className="w-6 h-6 md:w-8 md:h-8 text-rose-400" />
        </div>
        <div
          className="absolute top-20 right-16 animate-spin"
          style={{ animationDuration: "6s", animationDelay: "2s" }}
        >
          <Hash className="w-5 h-5 md:w-6 md:h-6 text-orange-400" />
        </div>
        <div
          className="absolute bottom-24 left-20 animate-spin"
        ...
        <div
          className="absolute bottom-1/4 right-1/4 w-64 h-64 md:w-80 md:h-80 bg-gradient-to-br from-orange-200/40 to-yellow-300/40 rounded-full blur-3xl animate-pulse"
      ...
      <div className="max-w-5xl mx-auto text-center relative z-10">
        ...
        <p className="text-base sm:text-lg md:text-xl text-gray-700 max-w-2xl mx-auto leading-relaxed mb-8">
          Transform your brand's online presence with strategic social media
          campaigns. Drive engagement, build communities, and convert followers
          into loyal customers across all major platforms.
        </p>
